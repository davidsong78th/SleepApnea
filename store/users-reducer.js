    ...
    switch (action.type) {
        case ADD_USERS:
            const newUser = new User(action.userData.id.toString(), action.userData.title, action.userData.ECGLog, action.userData.EEGLog,
                action.userData.OxymeterLog, action.userData.StrainLog, action.userData.FlowLog, action.userData.SnoreLog, action.userData.dateCreated)
            return {
        ...
        case SET_USERS:
            return {
                users: action.users.map(user => new User(user.id.toString(), user.title, user.ECGLog, user.EEGLog,
                    user.OxymeterLog, user.StrainLog, user.FlowLog, user.SnoreLog, user.dateCreated))
            }
        case DELETE_USERS:
            return {
